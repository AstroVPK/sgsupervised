...
arr = np.zeros((len(ids),), dtype=('i8, f4, f4, f4, f4'))
arr['f0'] = ids[:,2]; arr['f1'] = ras[:,2]; arr['f2'] = decs[:,2]
arr['f3'] = exts; arr['f4'] = pStar
header = 'id, RA (rads), Dec (rads), extendedness (mag_psf-mag_model), probability of being a star based on colors only'
np.savetxt('ucd.dat', arr, fmt='%d, %.18e, %.18e, %.18e, %.18e', header=header)
